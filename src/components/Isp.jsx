    ...
    <>
      ...
      <div ref={topRef} className={`fixed inset-[-1] right-6 bottom-6 p-1 cursor-pointer z-50 flex justify-center items-center ${navbarBg ? "" : "hidden"}`}>
        ...
        <div className="container mx-auto px-[16px] pt-[80px] pb-[240px] max-w-[1232px]">
          <div className="font-light text-[14px] mb-[80px] text-[#3973C5]"><a href='/#'><FormattedMessage id="isp_breadcrumbs_1"/></a> &gt; <FormattedMessage id="isp_breadcrumbs_2"/>  &gt; <FormattedMessage id="isp_breadcrumbs_3"/></div>
          <div className="font-bold text-[32px] mb-[24px]">
            <FormattedMessage id="isp_title"/>
          </div>
          <span id="contento" dangerouslySetInnerHTML={{ __html: intl.formatMessage({id: "isp_para"}) }} />
        </div>
        <div>
        </div>
      </div>
